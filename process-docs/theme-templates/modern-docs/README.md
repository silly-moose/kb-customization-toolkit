...
2. **Map the two key colors:**
   ...
   - **Then AA-check the accent as link text.** `--brand-link` defaults to the accent, and the accent is only AA-safe if it's a mid-dark color (the shipped `#0170b9` measures **5.22:1** on white). A **warm or light** brand accent — orange, amber, lime, yellow — will fail: an orange like `#ff5f14` measures **3.04:1** and is unreadable as body-link text. When it fails, set `--brand-link` to a darker readable variant (`var(--brand-accent-lo)`, which is `#015a94` at **7.26:1**, or a hand-picked hex such as `#c9490a` at **4.73:1**) and **leave `--brand-accent` bright** for the decorative roles. Measure with the helper in `../../01-KB_CUSTOMIZATION_PROJECT_SETUP.md` §4 rather than trusting a remembered figure. Record both values and note the deviation inline, so a later session doesn't "correct" the link color back to the brand hex.
