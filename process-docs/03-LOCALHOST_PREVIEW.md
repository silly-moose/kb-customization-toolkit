...
The same snapshot mechanism doubles as a **diagnostic tool** when a deployed change looks wrong and you'd rather not diagnose it against the live KB — e.g. the KB sits behind a reader login, or you want to test a fix before deploying it. (You can also sign into the KB in Claude's built-in browser and inspect the live page directly — see "Browser Tooling" in `CLAUDE-RULES.md`. Local reproduction still wins when you want to iterate on a change that isn't deployed yet.) Reproduce the deployed state locally and inspect it:
...
3. Serve it (`python3 -m http.server`) and open it with Claude's browser tooling to inspect **computed styles** — run `getComputedStyle` on the suspect elements. This is how you find the real cause fast: *which* wrapper is actually painting white, whether an element is `visibility:hidden`, what its real width/height is, etc.
...
**The converse also holds: measurements can't tell you a layout looks right.** They answer "is this element the width I intended," never "do these elements read as a coherent column." A per-element `max-width` is the classic case — cap the paragraphs at `88ch` while the `h1` and dividers stay full-width, and every measured number is correct while the page reads as ragged, with a large empty gutter. Rule of thumb: a per-element cap only looks right when **most siblings share it**. Measure for geometry, then **screenshot at the target viewport before handing the change over** (also in `CLAUDE-RULES.md` → Deployment Instructions).
