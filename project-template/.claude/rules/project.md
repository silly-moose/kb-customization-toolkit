# Project Info

- **Customer:** [customer name]
- **KB:** [knowledge base name or URL]
- **Deployment target:** [sandbox / live KB] — this determines the deployment instructions you write in CHANGES files
- **Toolkit path:** /Users/chadtimblin/My Drive (chad@knowledgeowl.com)/Claude Code desktop/Customers/*kb-customization-toolkit — where the central `improvement-log.md` lives, so end-of-session friction notes can be appended to it. If blank or unreachable, Claude won't create a log — it'll just suggest sharing any improvement ideas with Chad in Slack.

# Project Notes

<!-- Add any project-specific context here as it comes up: brand colors, design preferences, areas to avoid, special constraints, etc. -->
